...
class GapFilling(Simulator):
    ...
    def run_GapFill(self, target_reaction, flux_constraints={}, universal_reactions=[], inf_flag=False, solution_number=100):
        ...
        # m.setObjective(quicksum((b_bool[each_reaction]) for each_reaction in universal_reactions), GRB.MAXIMIZE)
        ...
        for each_solution in range(solution_number):
            ...
            if m.status == GRB.Status.OPTIMAL:
                ...
                ReactionFlux = {}
                print(v[target_reaction].x)
                for reaction in universal_reactions:
                    # if b_bool[reaction].x > 1e-3:
                    if b_bool[reaction].x > 1e-8:
                        print(reaction, b_bool[reaction].x)
